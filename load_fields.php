...
if(isset($_SESSION['login_type']) && $_SESSION['login_type'] == 2 && isset($_SESSION['login_id'])) {
	...
	$loan_check = $conn->query("SELECT borrower_id FROM loan_list WHERE id = '$loan_id'");
	if($loan_check && $loan_check->num_rows) {
		$borrower_id = $loan_check->fetch_assoc()['borrower_id'];
		if($borrower_id != $expected_borrower_id) {
